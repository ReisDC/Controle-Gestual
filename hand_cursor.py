...
class Configuracoes:
    ...
    limite_dedos_juntos_scroll: float = 0.06
    limite_delta_scroll: float = 0.01
    multiplicador_scroll: int = 800
...
def dedos_indicador_e_medio_levantados_juntos(pontos_mao, config: Configuracoes) -> bool:
    """Detecta indicador e médio esticados e próximos, usado para o scroll."""
    ponta_indicador = pontos_mao[8]
    junta_indicador = pontos_mao[6]
    ponta_medio = pontos_mao[12]
    junta_medio = pontos_mao[10]

    indicador_esticado = ponta_indicador.y < junta_indicador.y
    medio_esticado = ponta_medio.y < junta_medio.y
    dedos_juntos = distancia_entre_pontos(ponta_indicador, ponta_medio) < config.limite_dedos_juntos_scroll

    return indicador_esticado and medio_esticado and dedos_juntos
...
def principal() -> None:
    ...
    camera = None
    # Testa do índice configurado até o índice 4
    ...
    if camera is None:
        raise RuntimeError("Nenhuma câmera funcional foi encontrada.")
    ...
    scroll_y_anterior: float | None = None
    ...
    try:
        while True:
            ...
            if resultado.hand_landmarks:
                ...
                gesto_scroll_ativo = dedos_indicador_e_medio_levantados_juntos(pontos_mao, config)

                if gesto_scroll_ativo:
                    y_atual_scroll = (ponta_indicador.y + ponta_medio.y) / 2
                    if scroll_y_anterior is not None:
                        delta_y = y_atual_scroll - scroll_y_anterior
                        if abs(delta_y) > config.limite_delta_scroll:
                            pyautogui.scroll(int(-delta_y * config.multiplicador_scroll))
                    scroll_y_anterior = y_atual_scroll
                ...
                if not gesto_scroll_ativo:
                    scroll_y_anterior = None
